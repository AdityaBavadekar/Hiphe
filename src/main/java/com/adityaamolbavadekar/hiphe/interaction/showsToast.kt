...
object showsToast {
    ...
    private fun Context.showLongToast(text: String) {
        Toast.makeText(this, text, Toast.LENGTH_LONG).show()
    }

    private fun Context.showLongToastWithGravity(text: String) {
        val toast = Toast.makeText(this, text, Toast.LENGTH_LONG).apply {
            setTheme(R.style.Theme_MaterialComponents_Light)
            show()
        }
    ...
    private fun View.showSnackbarWithAction(text: String, action: String) {
        val snack = Snackbar.make(this, text, Snackbar.LENGTH_SHORT)
        snack.setAction(action) { snack.dismiss() }
        snack.show()
    }
